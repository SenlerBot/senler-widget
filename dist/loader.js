...
export function assertSenlerWidgetRuntime(value) {
...
export function loadSenlerWidget(options) {
    ...
    try {
        ...
        if (url.protocol !== "https:" && url.protocol !== "http:")
            throw new Error("SenlerWidget src must use HTTP or HTTPS");
        src = url.href;
    }
    catch (error) {
    ...
    if (pending) {
        if (pending.src !== src)
            return Promise.reject(new Error("A different SenlerWidget loader is already in use on this page"));
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        const check = () => {
            ...
            if (!api || Reflect.get(api, "__isBootstrapStub"))
                return false;
            clean();
            try {
    ...
    void promise.catch(() => {
        if (loads.get(target)?.promise === promise)
            loads.delete(target);
    });
